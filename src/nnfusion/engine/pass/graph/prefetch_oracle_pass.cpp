...
#include "prefetch_oracle_pass.hpp"
#include <queue>
...
DEFINE_bool(fprefetch_oracle_pass,
            false,
...
bool PrefetchOraclePass::run_on_graph(std::shared_ptr<Graph>& graph)
{
    if (!FLAGS_fprefetch_oracle_pass)
        return true;

    NNFUSION_LOG(INFO) << "Prefetching(Oracle) graph: " << graph->get_name();
    ...
    for (size_t index = 0; index < kernel_node_vec.size(); ++index){
        ...
        // Add same gnode to graph, prefetch_cur_gnode -> cur_gnode
        // auto prefetch_op = std::make_shared<nnfusion::op::NoOp>("prefetch_" + cur_gnode_in_graph->get_name());
        // GNodeVector empty_inputs;
        // auto prefetch_node = std::make_shared<nnfusion::graph::GNode>(prefetch_op, empty_inputs);

        // graph->add_node(prefetch_node);
